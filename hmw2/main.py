...
filename = 'ift6285/hmw2/test1000sent'

with open(filename, 'r') as f:
    lines = f.readlines()


# model = kenlm.Model('ift6285/hmw2/output.mmap')
model = kenlm.Model('ift6285/hmw2/output01-01-o2.mmap')
# print('perplexity', model.perplexity(lines))
# print('score', model.score(lines, bos = True, eos = True))
perplexity_list = []
score_list = []

for line in lines:
    perplexity_list.append(model.perplexity(line))
    score_list.append(model.score(line))

perplexity_np = np.array(perplexity_list)
score_np = np.array(score_list)
# print('average entropy score', np.round(np.mean(score_np),2))
# print('min entropy score', np.round(np.min(score_np), 2))
# print('max entropy score', np.round(np.max(score_np),2))

print('average perplexity', np.round(np.mean(perplexity_np),2))
print('min perplexity', np.round(np.min(perplexity_np), 2))
print('max perplexity', np.round(np.max(perplexity_np),2))


# print('perplexity', model.perplexity(test))


# print('score', model.score(test, bos = True, eos = True))
...
#     for line in lines:
        ...
        # args = "/u/demorali/bin/x86_64/moses_3.0/bin/lmplz -o 5 </u/demorali/corpora/1g-word-lm-benchmark-r13output/training-monolingual.tokenized.shuffled/news.en-00001-of-00100".split()
        ...
        #print(output)
        # model = kenlm.Model('ift6285/hmw2/text.arpa')
        # model = kenlm.Model('ift6285/hmw2/output.mmap')
        # print('perplexity', model.perplexity('ift6285/hmw2/test1000sent'))
        # print('score', model.score('this is a sentence .', bos = True, eos = True))
